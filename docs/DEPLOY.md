# VPS Deploy — Pepperbot

VPS: `root@76.13.218.127` (Ubuntu 24.04)
Target dir: `/opt/pepperbot`

## 用法

打开本文件 → 选中下面 ```bash 代码块的**全部内容**（从 `#!/bin/bash` 到末尾 `====`） → 拷贝 → SSH 到 VPS → 粘贴 → 回车。

预计耗时：装 docker (~2 分钟) + clone (~10 秒) + build image (~3-5 分钟) + smoke (~5 秒) = 一共 ~8 分钟。
...
应该看到 `pepperbot` 容器 `Up`。

```bash
docker compose logs --tail=20
```

应该看到 entrypoint 跑 migrations + cron 启动。

## 常见问题

- **`docker compose` 不识别 `compose`**：旧版 docker。脚本里的 `curl get.docker.com` 装的是新版，应该没事；如果还是炸，跑 `apt-get install docker-compose-plugin`。
- **build 阶段拉镜像超慢**：Playwright base 镜像 ~500MB，第一次拉慢正常。VPS 在国外会快很多。
- **migrations 失败**：贴最后 20 行错误。多半是 SQL syntax 或 schema 冲突。
- **smoke test `db_ok=false`**：数据库挂载没生效，检查 `/opt/pepperbot/data` 权限。

## 部署后立即做的事

1. **轮转所有泄露的 secret**（Discord token / 3 个 cookie 都在聊天里贴过）
2. **试一次端到端**：

   ```bash
   docker compose exec pepperbot python -m src.main observe
   docker compose exec pepperbot python -m src.main post
   ```

   post 那条如果生成了草稿应该会推到 Discord 频道。

3. **看告警**：故意把某个 cookie 文件改坏，跑 `observe` → 应该收到 Discord 告警。
